...
Shader::Shader(const std::string& filePath, const std::initializer_list<const std::string>& paramNames, ui noParams, const std::initializer_list<const std::string>& uniformNames, ui noUniforms, bool hasGeometryShader)
{
    ...
    for (ui i = 0; i < noParams; ++i)
        glBindAttribLocation(program, i, paramNames.begin()[i].c_str());
    ...
    for (ui i = 0; i < noUniforms; ++i)
        uniforms[i] = glGetUniformLocation(program, uniformNames.begin()[i].c_str());
}
